...
All services run as Docker containers managed by `docker-compose`. Reverse proxy (Nginx) routes subdomains to internal ports, handles HTTPS via Let's Encrypt. No code changes to the C engine — `ska_trading_bot.py` writes to QuestDB in addition to CSV.
