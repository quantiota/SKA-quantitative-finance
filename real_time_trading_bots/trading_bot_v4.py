...
Changelog:
  ...
  v4: Requires own pair confirmation before listening for exit (hybrid)
      - Wait for bull→neutral (LONG) or bear→neutral (SHORT) before listening for close
      - Once pair confirmed, exit on the FIRST opposite signal (neutral→bear OR bear→neutral)
      - Combines v3 structural entry quality with v1 exit speed

Signal logic — full cycle:

  LONG:
    neutral→bull               (OPEN LONG — UP cycle opens)
    bull→neutral               (UP pair confirmed — NOW listen for exit)
    neutral→bear OR bear→neutral  (CLOSE LONG — first opposite signal)

  SHORT:
    neutral→bear               (OPEN SHORT — DOWN cycle opens)
    bear→neutral               (DOWN pair confirmed — NOW listen for exit)
    neutral→bull OR bull→neutral  (CLOSE SHORT — first opposite signal)

State machine per position:
  WAIT_PAIR_CONFIRM  → waiting for own pair confirmation before exit is active
  READY_TO_EXIT      → pair confirmed, exit on first opposite signal

The alpha: price follows paired regime transitions.
Entry waits for complete own cycle. Exit fires on first opposite structural signal.
"""
...
class SKATradingBot:
    """Paired cycle trading bot v4 — pair confirmation required, first opposite signal exits."""
    ...
    def __init__(self, db_host='192.168.1.216', db_port=8812, symbol='XRPUSDT',
        ...
        self.results_file = f'/home/coder/project/Real_Time_SKA_trading/bot_results_v4/bot_results_v4_{ts}.csv'
    ...
    def process_signal(self, transition):
        ...
        LONG:
          WAIT_PAIR_CONFIRM → bull→neutral       → READY_TO_EXIT
          READY_TO_EXIT     → neutral→bear        → CLOSE LONG
                            → bear→neutral        → CLOSE LONG

        SHORT:
          WAIT_PAIR_CONFIRM → bear→neutral       → READY_TO_EXIT
          READY_TO_EXIT     → neutral→bull        → CLOSE SHORT
            ...
            if self.position.exit_state == WAIT_PAIR_CONFIRM:
                if name == 'bull→neutral':
                    ...
                    logging.info(
                        ...
                        f"| READY TO EXIT | trade_id={trade_id}"
                    )
                ...
                if name in ('neutral→bear', 'bear→neutral'):
                    ...
                    # If exit was neutral→bear, open SHORT immediately
        ...
        # === SHORT POSITION ===
            ...
            if self.position.exit_state == WAIT_PAIR_CONFIRM:
                ...
                if name in ('neutral→bull', 'bull→neutral'):
                    ...
                    # If exit was neutral→bull, open LONG immediately
                    if name == 'neutral→bull':
                        self.position = Position(
                            side='LONG', entry_price=price,
                            entry_trade_id=trade_id, entry_time=str(ts),
    ...
    async def run(self, max_trades=3500):
        await self.connect()
        logging.info(f"SKA Trading Bot v4 | symbol={self.symbol} | dry_run={self.dry_run} | auto_stop={max_trades}")
        logging.info("LONG:  neutral→bull → bull→neutral (confirmed) → neutral→bear OR bear→neutral (CLOSE)")
        logging.info("SHORT: neutral→bear → bear→neutral (confirmed) → neutral→bull OR bull→neutral (CLOSE)")
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='SKA Paired Regime Trading Bot v4')
