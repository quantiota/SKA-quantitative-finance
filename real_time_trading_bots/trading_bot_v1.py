...
class SKATradingBot:
    ...
    def __init__(self, db_host='192.168.1.216', db_port=8812, symbol='XRPUSDT',
        ...
        self.results_file = f'/home/coder/project/Real_Time_SKA_trading/bot_results_v1_{ts}.csv'
    ...
    def process_signal(self, transition):
        ...
        if self.position.side == 'LONG':
            ...
            # Opposite side appears: cycle complete → CLOSE LONG
                ...
                logging.info(
                    f"<<< CLOSE LONG (cycle complete: {name}) @ {price:.6f} | "
                ...
                # If the exit signal is neutral→bear, also open SHORT
                if name == 'neutral→bear':
                    ...
                    self.pair_confirmed = False
                    logging.info(
                        f">>> OPEN SHORT (new cycle) @ {price:.6f} | trade_id={trade_id}"
                    )
                    if not self.dry_run:
                        self._execute_sell(price)
        ...
        # === SHORT POSITION ===
            ...
            # Opposite side appears: cycle complete → CLOSE SHORT
            elif name in ('neutral→bull', 'bull→neutral'):
                pnl = self.position.entry_price - price
                pnl_pct = (pnl / self.position.entry_price) * 100
                self._record_trade(pnl, pnl_pct, price)
                logging.info(
                    f"<<< CLOSE SHORT (cycle complete: {name}) @ {price:.6f} | "
                    f"PnL={pnl:+.6f} ({pnl_pct:+.4f}%) | entry={self.position.entry_price:.6f}"
                )
                if not self.dry_run:
                    self._execute_buy(price)
                ...
                # If the exit signal is neutral→bull, also open LONG
                if name == 'neutral→bull':
                    self.position = Position(
                        side='LONG', entry_price=price,
                        entry_trade_id=trade_id, entry_time=str(ts),
                    ...
                    self.pair_confirmed = False
                    logging.info(
                        f">>> OPEN LONG (new cycle) @ {price:.6f} | trade_id={trade_id}"
                    )
                    if not self.dry_run:
                        self._execute_buy(price)
